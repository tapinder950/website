...
export default function MemberCard({ refresh, searchTerm = "", isCompact = false }: MemberCardProps) {
  const { gymId } = useGym()
  ...
  const filteredMembers = useMemo(() => {
    ...
    return filtered
  }, [members, searchTerm, localSearch, filterBy, sortBy])

  const paginatedMembers = useMemo(() => {
    if (isCompact) return filteredMembers.slice(0, ITEMS_PER_PAGE)
    
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
    return filteredMembers.slice(startIndex, startIndex + ITEMS_PER_PAGE)
  }, [filteredMembers, currentPage, isCompact])
    ...
    <div className={cn(
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-1 ml-2">
          <EditMemberDialog member={member} onUpdated={fetchMembers} />
          <DeleteDialog
            label="Member"
            onConfirm={async () => {
              // Security: Only delete if member belongs to this gym
              await supabase.from("members").delete().eq("id", member.id).eq("gym_id", gymId)
              handleDeleted()
            }}
          />
        </div>
        ...
        <CardHeader className="pb-4">
          ...
          <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg sm:text-xl">Member Management</CardTitle>
            ...
            {filteredMembers.length === 0 && (
              <div className="text-center py-8 sm:py-12">
                ...
                <p className="text-sm text-gray-600 mb-4 px-4">
                  {searchTerm || localSearch ? 'Try adjusting your search terms' : 'Get started by adding your first member'}
                </p>
                <AddMemberDialog onAdded={fetchMembers} />
              </div>
            )}
          ...
          {!isCompact && totalPages > 1 && (
            <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:justify-between sm:items-center mt-6 pt-4 border-t">
              <p className="text-xs sm:text-sm text-gray-600 text-center sm:text-left">
                Showing {Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, filteredMembers.length)} to{' '}
